...
def plot(data: dict[str, list[int | datetime]]) -> None:
    ...
    df['15min_block'] = df['time'].dt.floor('15min')  # die Daten in 15-Minuten-Blöcke einteilen
    df = df.groupby('15min_block')['delay'].mean().reset_index()  # für jeden 15-Minuten-Block den Durchschnitt berechnen
    plt.figure(figsize=(9, 6))  # 900 mal 600 Pixel groß
    plt.plot(df['15min_block'], df['delay'], color='r', linewidth=1, label='Verspätungen')
    plt.title(f'Durchschnittliche Verspätungen am {TARGET_DAY.strftime('%d.%m.%Y')} in 15-Minuten-Blöcken')  # der Titel des Plots
